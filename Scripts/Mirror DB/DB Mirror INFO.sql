...
ORDER BY DB_NAME(database_id);--SCRIPT TO INDICATE WHICH DATABASE IS IN SYNC MODE
...
SELECT
'ALTER DATABASE [' + DB_NAME(database_id) + '] SETPARTNER SAFETY OFF;'
...
WHERE 1=1
AND mirroring_guid IS NOT NULL
AND mirroring_role_desc = 'PRINCIPAL'AND mirroring_state_desc <> 'SUSPENDED'
ORDER BY DB_NAME(database_id);
...
--alter database mirror set partner offrestore database mirror
